...
def dashboard_admin():
    ...
    # Ambil semua laporan dari PostgreSQL
    query = """
    ...
    for index, report in reports.iterrows():
        with st.container():
            ...
            st.caption(f"👤 Pengirim: {report['username']} | 🕒 {report['timestamp']}")
            st.caption(f"📦 Mode: `{report['mode']}`")

            # =============== Description (Encrypted + Decrypt Button)
            if report['description']:
                with st.expander("Lihat / Dekripsi Deskripsi"):
                    st.code(report['description'])
                    if st.button(f"Decrypt Description #{report['id']}", key=f"desc_{report['id']}"):
                        try:
            ...
            if report['file_path']:
                ...
                if st.button(f"Decrypt File(s) #{report['id']}", key=f"file_{report['id']}"):
                    for fpath in file_list:
                        ...
                        try:
                            ...
                            # Download langsung (pakai buffer)
            ...
            if report['mode'] == "steganografi" and report['image_path']:
                st.write("🖼️ **Steganography Image:**")
                if os.path.exists(report['image_path']):
                    st.image(report['image_path'], caption="Image with Hidden Message", width=300)
                    if st.button(f"Decode Stego #{report['id']}", key=f"stego_{report['id']}"):
                        try:
